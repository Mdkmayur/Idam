...
export default async function SriLankaProjectDetail({ params }: { params: { slug: string } }) {
  const project = await prisma.project.findFirst({
    where: { country: 'SRILANKA', slug: params.slug },
  ...
  const candidates = await prisma.project.findMany({
    where: { country: project.country, id: { not: project.id } },
    select: {
      id: true,
      title: true,
      summary: true,
  ...
  const related = candidates
    .map((p) => {
      ...
      const score = sameDivision + featured + tagScore * 2
      return { p, score }
    })
    .sort((a, b) => {
  ...
  return (
    <main className="max-w-6xl mx-auto px-6 py-10">
      <ProjectViewTracker projectId={project.id} country={project.country} />
      <div className="text-xs tracking-[0.35em] text-idam-gold">SRI LANKA • {project.division.name}</div>
      ...
      <div className="mt-10 prose-premium">
        <h2>Overview</h2>
      ...
      {gallery.length ? (
        <section className="mt-10">
          <div className="flex items-end justify-between gap-3">
            ...
            <Link className="text-sm text-idam-navy hover:underline" href={`/srilanka/projects/${project.slug}/gallery`}>
              View all →
            </Link>
          </div>
          <div className="mt-6">
